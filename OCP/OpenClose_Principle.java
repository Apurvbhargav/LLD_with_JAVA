...
class UpiPayment extends Payment {
    ...
    void pay() {
        for (Product p : cart.getProduct()) {
            System.out.println("Paying for " + p.name + " with UPI");
        }
...
class CreditCardPayment extends Payment {
    ShoppingCart cart;
    public CreditCardPayment(ShoppingCart cart) {
        this.cart = cart;
    }
    ...
    void pay() {
        for (Product p : cart.getProduct()) {
            System.out.println("Paying for " + p.name + " with Credit Card");
        }
...
public class OpenClose_Principle {
    public static void main(String[] args) {
        ...
        Payment payment = new UpiPayment(cart);
        payment.pay();
        Payment payment2 = new CreditCardPayment(cart);
        payment2.pay();
    }
}
